...
> [!TIP]
> **Best Practice**: Use `--headed` and `--step-by-step` during development to see exactly where a test fails and inspect the page state manually.

## Why Run Locally?

1. **Zero Cost**: Local runs do not consume cloud execution credits (though they may consume small amounts of AI reasoning credits depending on your plan).
...
> [!IMPORTANT]
> Ensure your local machine has a stable internet connection. While the browser is local, the "intelligence" that drives the agent resides in the Rova cloud.
